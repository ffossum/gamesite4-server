...
import           Gamesite4.Parsers              ( WsRoomMsg(..)
                                                , parseRoomName
                                                , parseUsername
                                                , parseWsRoomMsg
                                                )
...
handleUserRedisMsg pubSubCtrl channel (Username username) action = bracket
...
receiveGameMsgs stateVar redisChannel bytes = atomically $ do
...
removeRoomChannel roomId clientState = pure
  $ clientState { roomChannels = newRoomChannels }
 where
...
respondForever client channelsState = forever $ do
  msg <- atomically $ do
...
receiveForever client redisConnection pubSubCtrl clientStateVar stateVar =
 ...
 where
  loop = do
    ...
    case msg of
      _
        | "/join " `T.isPrefixOf` msg -> case parseRoomName msg of
          Nothing     -> putStrLn "invalid join room request"
          Just roomId -> atomically $ do
            state <- readTVar stateVar
            ...
            case M.lookup roomId rooms of
              ...
              Just room -> do
                ...
                writeTVar
                  stateVar
                  (state
                    { serverStateRooms = M.adjust incrementClients roomId rooms
                    }
                  )
        | "/leave " `T.isPrefixOf` msg -> case parseRoomName msg of
                ...
                in
                  if clients > 1
                    then writeTVar
                      stateVar
                      (state
                        { serverStateRooms = M.adjust decrementClients
                                                      roomId
                                                      rooms
                        }
                      )
                      ...
                      stateVar
                      (state { serverStateRooms = M.delete roomId rooms })
        | "#" `T.isPrefixOf` msg -> case parseWsRoomMsg msg of
          Nothing -> pure ()
          Just (WsRoomMsg roomName content) ->
            R.runRedis
                redisConnection
                (R.publish (roomNameToRedisChannel roomName)
                           (encodeUtf8 content)
                )
              *> pure ()
        | "user:" `T.isPrefixOf` msg -> do
          _ <- R.runRedis redisConnection
            $ R.publish "user:123" (encodeUtf8 msg)
          pure ()
        | otherwise -> pure ()
